...
generateRandomString <- function(length = 10) {
  ...
  # Randomly sample characters and paste them together
...
makeHeatmapDeleteomeMatches <- function(mutantname=NA,             # Name of deletion strain (used for plot titles and file names)
  ...
  clust <- heatmap.2(mybigmat, Colv = clusterColumns, trace = "none", symbreaks = T, 
   ...
   # Extract row and column ordering used in heatmap
  row_order <- clust$rowInd      # Index order of genes
  col_order <- clust$colInd      # Index order of strains

  # Map these orders to gene and strain names
  y_axis_genes <- rownames(mybigmat)[row_order]   # Ordered list of genes on the Y-axis
  x_axis_strains <- colnames(mybigmat)[col_order] # Ordered list of strains on the X-axis
  
  # Get coordinates as a data frame
  gene_strain_coordinates <- expand.grid(
    Gene = y_axis_genes,
    Strain = x_axis_strains
  )

  # Adding the matrix values as an additional column for reference
  gene_strain_coordinates$ExpressionValue <- as.vector(mybigmat[row_order, col_order])

  if(printToFile){ 
    dev.off()
    message("Printed heatmap to ", hmfile)
  }
  # Retrieve Y-axis gene names
y_axis_genes <- rownames(mybigmat)
print("y_axis_genes")
print(y_axis_genes)

print("gene_strain_coordinates")
print(gene_strain_coordinates)
  # Export coordinates to Excel
  excel_file <- paste0(mutantname, "_", fileprefix, "_gene_strain_coordinates.xlsx")
  write_xlsx(gene_strain_coordinates, path = excel_file)
  message("Exported coordinates to ", excel_file)
  return(mybigmat)
}
